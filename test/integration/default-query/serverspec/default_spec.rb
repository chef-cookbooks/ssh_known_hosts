...
describe 'Known Hosts' do
  ...
  describe file('/etc/ssh/ssh_known_hosts') do
    ...
    its(:content) { should_not include "ecdsa-sha2-nistp256 TEST_ECDSA_PUBLIC_KEY_HOST-2\n" }
    its(:content) { should_not include "ssh-ed25519 TEST_ED25519_PUBLIC_KEY_HOST-3\n" }
  end
end
